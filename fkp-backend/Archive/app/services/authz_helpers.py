...
# Role-role non-superadmin yang punya akses global tanpa terikat hierarki wilayah
# (section 3.6 dokumen). "superadmin" TIDAK masuk di sini — superadmin selalu
# dicek lewat is_superadmin() di has_global_access() yang dipanggil SEBELUM
# cek set ini, sehingga string "superadmin" di set ini hanya redundan dan
# berpotensi menyesatkan: role baru dengan is_superadmin=True tapi kode_role
# bukan "superadmin" akan tetap lolos lewat jalur is_superadmin(), bukan lewat
# set ini.

# [FIX BUG KRITIS #3 — audit modul Sample Shipment/Warehouse] "warehouse"
# ditambahkan. Sebelumnya tidak ada di sini, sehingga get_scoped_distributor_ids()
# jatuh ke cabang "role tidak dikenal -> []" untuk warehouse, dan
# upload_service.upload_attachment() (yang memanggil has_global_access() lalu
# assert_distributor_in_scope() kalau False) SELALU menolak upload dari role
# warehouse dengan 403 — termasuk upload dokumen sample (tanda_terima_sample,
# foto_kondisi_masuk) yang justru salah satu alur kerja utama role ini.
# Konsisten dengan _ROLE_GLOBAL_ACCESS di fkp_service.py yang harus disinkronkan.
GLOBAL_ACCESS_ROLES = {"admin_ho", "qc", "rsm", "direktur", "finance", "warehouse"}
