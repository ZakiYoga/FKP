...
async def upload_attachment(
    ...
    sample_shipment_id: Optional[uuid.UUID] = None,
) -> FkpAttachment:
    ...
    # [BARU — Sample Shipment] Validasi sample_shipment_id milik FKP ini
    if sample_shipment_id is not None:
        r_sample = await db.execute(
            select(SampleShipment).where(
                SampleShipment.id == sample_shipment_id,
                SampleShipment.fkp_id == fkp_id,
            )
        )
        if not r_sample.scalar_one_or_none():
            raise HTTPException(
                status_code=404,
                detail="Sample shipment tidak ditemukan atau tidak termasuk dalam FKP ini.",
            )
    ...
    # Buat subfolder per FKP (dan per item/sample jika ada)
    if sample_shipment_id:
        fkp_dir = os.path.join(settings.UPLOAD_DIR, "fkp", str(fkp_id), "samples", str(sample_shipment_id))
        url = f"/uploads/fkp/{fkp_id}/samples/{sample_shipment_id}/{unique_name}"
    elif fkp_item_id:
    ...
    attachment = FkpAttachment(
        ...
        sample_shipment_id=sample_shipment_id,
